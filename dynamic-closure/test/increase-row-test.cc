...
namespace {
class IncreaseRowTest : public ::testing::Test {
   protected:
...
TEST_F(IncreaseRowTest, DimensionsMismatch) {
    ...
    // Calling decrease
    GrB_Info info =
        DyC_IncreaseRow(&F, dyc_semiring, A, S, &X, 1, &I, 1, J, 2, msg);
    EXPECT_EQ(info, GrB_DIMENSION_MISMATCH);
}

TEST_F(IncreaseRowTest, InvalidIndex) {
    char msg[DYC_MSG_LEN];
    GrB_Matrix F, A, S;
    ...
    // Calling decrease
    GrB_Info info =
        DyC_IncreaseRow(&F, dyc_semiring, A, S, X, 2, I, 2, J, 2, msg);
    EXPECT_EQ(info, GrB_INVALID_INDEX);
}

TEST_F(IncreaseRowTest, MinPlus) {
    char msg[DYC_MSG_LEN];
...
TEST_F(IncreaseRowTest, ExtendedMinPlus) {
    char msg[DYC_MSG_LEN];
    GrB_Matrix F, A, US, S;
