...
int main(int argc, char* argv[]) {
    ...
    char *dataset, *s_dataset, *out_dir;
    int xnvals = 1;
    ...
    printf(ANSI_COLOR_YELLOW
        "o" ANSI_COLOR_RESET
        " - DynamicClosure: benchmarking DyC_DecreaseIdempotent.c, Semiring: "
        "MIN PLUS FP64\n"
        "\t - Dataset: %s\n"
        "\t - Number of updates: %d\n",
        dataset, xnvals);
    ...
    double dynamic_closure_wall, distrust_matrix_wall, closure_wall = 0;
    ...
    LAGRAPH_TRY(DYC_GenerateUpdates(&X, &I, &J, xnvals, 10, 100, nodes));
    ...
    LAGRAPH_TRY(
        DyC_DistrustMatrix(&D, dyc_semiring, GrB_EQ_FP64, A, S, I, xnvals, J, xnvals, msg));
    ...
    wall_end = LAGraph_WallClockTime();
    distrust_matrix_wall = wall_end - wall_start;
    ...
    LAGRAPH_TRY(DyC_DecreaseIdempotent(
        &F, dyc_semiring, A, S, D, X, xnvals, I, xnvals, J, xnvals, msg));
    ...
    if (out_dir != NULL) {
        ...
        asprintf(&filename, "%s/D.txt", out_dir);
        DYC_MMPrint(D, filename, out_dir);
        ...
        asprintf(&content, "%s,DyC_DistrustMatrix,%d,%f\n", "D", true,
            distrust_matrix_wall);
        DYC_AnalyticsWrite(content, out_dir);
        asprintf(&content, "%s,DyC_DecreaseIdempotent,%d,%f\n", "F", true,
